...
class Analysis_prior_net(tf.keras.Model):
    def __init__(self):
        super(Analysis_prior_net, self).__init__()
        self.conv1 = tf.keras.layers.Conv2D(10,3, strides=(1, 1), padding='same', data_format="channels_first")
        self.relu1 = tf.keras.layers.ReLU()
        self.conv2 = tf.keras.layers.Conv2D(10,5, strides=(1, 1), padding='same', data_format="channels_first")
        self.relu2 = tf.keras.layers.ReLU()
        self.conv3 = tf.keras.layers.Conv2D(10,5, strides=(1, 1), padding='same', data_format="channels_first")
...
def build_model():
    input_image = torch.zeros([5, 3, None, None])
