...
class Synthesis_prior_net(tf.keras.Model):
    def __init__(self):
        super(Synthesis_prior_net, self).__init__()
        self.deconv1 = tf.keras.layers.Conv2D(10,5, strides=(1, 1), padding='same', data_format="channels_first")
        self.relu1 = tf.keras.layers.ReLU()
        self.deconv2 = tf.keras.layers.Conv2D(10,5, strides=(1, 1), padding='same', data_format="channels_first")
        self.relu2 = tf.keras.layers.ReLU()
        self.deconv3 = tf.keras.layers.Conv2D(1,3, strides=(1, 1), padding='same', data_format="channels_first")
...
def build_model():
    input_image = torch.zeros([7,3,None,None])
